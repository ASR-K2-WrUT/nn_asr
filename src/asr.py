...
def save_stats( stats, file, phone_file ):
    ...
    if ( os.path.isfile( phone_file ) ):
        ...
        print( "WARNING: phone file %s does not exist. Default names used" % phone_file )
        for i in range( len( histo ) ):
           phones.append ( "ph%02d"%i );  
        phones.append( "UNK" );           
    ...
    if ( len(path) != len(phones) - 1 ):
       print( "Error in lookup size: phones(%d)   path(%d)" % (len(path), len(phones)) );
