...
# Number of segments in input data file the segment names are basename.[2nd]*.seg<nnn>.bin
data_segments_cnt = -1
...
def prepare_2nd_order_data( X_data, y_data, L, src_fname, nnout_fn, seg_index ):
    ...
    for pos in range( tr_size ):
        lin_pos = 0;
        for ctx in range( -ctx_width, ctx_width+1 ):
            src_pos = pos + ctx
            if ( pos + ctx < 0 ):
                src_pos = 0;
            else:
    ...
    out_file = t[0] + (".2nd.seg%03d" % seg_index) + extension
...
def main(model='mlp', num_epochs=500):
    ...
    # Determine count of train data segments
    train_full_fname = sys.argv[3];
    data_seg_names = []
    for seg in range(1000):
        t = pth.splitext( train_full_fname );
        out_file = t[0] + (".seg%03d.bin" % seg );
        if (not os.path.isfile( out_file )):
           break;
        data_seg_names.append( out_file );
        print ( "Appended seg file " + out_file )
    ...
    if ( use_bulk ):
       ...
       if ( len(data_seg_names) == 1):       
           X_train_dummy, y_train_dummy, pos_train, X_val, y_val, pos_val, X_test_dummy, y_test_dummy, pos_test_dummy = \
              load_dataset_crafted( data_seg_names[0], sys.argv[4], "" )
       else:
           X_train_dummy, y_train_dummy, pos_train_dummy, X_val, y_val, pos_val, X_test_dummy, y_test_dummy, pos_test_dummy = \
              load_dataset_crafted( "", sys.argv[4], "" )
    ...
    for epoch in range(num_epochs):
        ...
        shuffle( data_seg_names );
        data_seg_names_copy = data_seg_names[:]
        shuffle( data_seg_names_copy );
        seg_tested = 0
        for data_seg_name in data_seg_names_copy:
            # Load data_segments_cnt
            seg_tested = seg_tested + 1
            print ( "testing with segment " + str( seg_tested ) + " of " + str( len( data_seg_names ) ) )
            if ( len(data_seg_name) > 1):
                if ( not first_iter ):
                   first_iter = False;
                   del X_train
                   del y_train
                   del pos_train
                print ( "Loading train data from: " + data_seg_name )
                start_time = time.time()
                X_train, y_train, pos_train, \
                    X_val_dummy, y_val_dummy, pos_val_dummy, \
                    X_test_dummy, y_test_dummy, pos_test_dummy = load_dataset_crafted( data_seg_name, "", "" )
            ...
            print("Load/train time  {:.3f}s / {:.3f}s    ratio: {:.3f}".format(load_time, train_time, load_time/train_time ))
    ...
    del y_val_dummy
    del pos_val_dummy
    X_train_dummy, y_train_dummy, pos_train_dummy, X_val_dummy, y_val_dummy, pos_val_dummy, X_test, y_test, pos_test = \
              load_dataset_crafted( "", "", sys.argv[5] )    
    test_err = 0
    test_acc = 0
    test_batches = 0
    ...
    if ( positional_data_ready ):     
        ...
        prepare_2nd_order_data( X_test,  y_test, pos_test, sys.argv[5], nnout_fn );
        del X_test
        del y_test
        del pos_test
        ...
        X_train_dummy, y_train_dummy, pos_train_dummy, X_val, y_val, pos_val, X_test_dummy, y_test_dummy, pos_test_dummy = \
              load_dataset_crafted( "", sys.argv[4], "" )            
        prepare_2nd_order_data( X_val, y_val, pos_val, sys.argv[4], nnout_fn );
        del X_val
        del y_val
        del pos_val
        ...
        print( "Loading from preselected sets" );    
        seg_index = 0
        for data_seg_name in data_seg_names:
           X_train, y_train, pos_train, \
              X_val, y_val, pos_val, \
              X_test, y_test, pos_test = load_dataset_crafted( data_seg_name, "", "" )        
           prepare_2nd_order_data( X_train, y_train, pos_train, data_seg_name, nnout_fn, seg_index );
           del X_train
           del y_train
           del pos_train
