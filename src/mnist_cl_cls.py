...
def prepare_2nd_order_data( X_data, y_data, L, src_fname, nnout_fn ):
...
def main( argv ):
    ...
    if ( len(argv) < 4 );
       print ("Usage")
       print ("cnn_arr <train_file> <valid_file> <test_file> <side_context> [RD <lookup_fname>] ")       
       return None;
    ...
    X_train, y_train, pos_train, X_val, y_val, pos_val, X_test, y_test, pos_test = load_dataset_crafted( train_fname, valid_fname, "" )
    positional_data_ready = True;
    
    
    # Build network and training functions
    # ==================================================================
    ...
    # Compile the function for exact NN outputs acquisition
    nnout_fn = theano.function( [input_var], test_prediction, allow_input_downcast=True)
    
    # Train the created network
    # =============================================================================
    ...
    for epoch in range(num_epochs):
        ...
        # Terminate the traininig loop if no improvements were obtained for a long time       
