...
def load_dataset( train_fn, valid_fn, test_fn ):
    global rows_cnt;
    global features_cnt;
    global output_size;
    ...
    X_train, y_train = load_clarin( train_fn );
    X_val, y_val     = load_clarin( valid_fn );
    X_test, y_test   = load_clarin( test_fn );    
    

    tr_size, layer_cnt, rows_cnt, features_cnt = X_train.shape;
    output_size = np.max( y_train ) + 1;
    
    print ("Read sizes: %d %d" % (rows_cnt, features_cnt) );
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.InputLayer(shape=(None, 3, rows_cnt, features_cnt),
                                        input_var=input_var)
...
def main(model='mlp', num_epochs=500):
    ...
    X_train, y_train, X_val, y_val, X_test, y_test = load_dataset( sys.argv[3], sys.argv[4], sys.argv[5] )
