...
def main( params ):
    ...
    for epoch in range(num_epochs):
        ...
        if ( val_result > max_acc + 0.0001 ):
           ...
           if ( epoch > 0 ) or ( epoch == 0 ):
               np.savez( nn_fname, *lasagne.layers.get_all_param_values(network));
               print( "  net saved" );
    ...
    # the faurther part of the scrip creates the training data for stacked DNN. It is experimental
    # only and currently is disabled.
    sys.exit()
    ...
    # inputs to the next NN. Each NN in the stack is trained in order to correctly solve the underlying
